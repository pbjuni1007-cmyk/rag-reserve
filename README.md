...
이 저장소는 팀 점검용 **진행 상황 스냅샷**입니다. 최신 구현·설계·보고서를 반영했으며, 점검 후 팀원별 제출을 진행합니다. [현재 상태와 점검할 내용](PROGRESS.md)을 먼저 확인해 주세요.
...
결과는 `outputs/<실행ID>/`의 `report.md`, `citation_review.md`, `gap_review.md`에 저장됩니다. 주장·인용·출처·실행 설정도 JSON으로 보존합니다. `human_review_pending`은 생성과 자동 검증을 마치고 인용 의미 검수를 기다리는 상태입니다. PDF 변환은 Markdown 검수 후 진행합니다.
